...
class TestForms(TestCase):
    ...
    def test_if_ticket_can_be_created(self):
        ...
        instance = Ticket(author=user,
        username=user.username,
        ticket_type=2, comment_num=0, upvotes=0)
    ...
    def test_ticket_form_requirements(self):
        ...
        instance = Ticket(author=user, username=user.username,
        ticket_type=2, status=1, comment_num=0, upvotes=0)
        
        form = TicketForm({'title': '', 'description': 'Test'},
        instance=instance)
        self.assertFalse(form.is_valid())
        
        form2 = TicketForm({'title': 'Test title', 'description': ''},
        instance=instance)
        self.assertFalse(form2.is_valid())
        
        form3 = TicketForm({'title': 'Test title', 'description': 'Test',
        'ticket_type': '2', 'status': 1},
        instance=instance)
...
# Comment Form Tests
    ...
    def test_if_comment_can_be_created(self):
        ...
        ticket = Ticket(author=user, username=user.username,
        ticket_type=2, status=1, comment_num=0,
        upvotes=0, title='Test', description='test')
        ticket.save()

        instance = Comment(username=user.username,
    ...
    def test_comment_form_requirements(self):
        ...
        form2 = CommentForm({'comment': 'test comment'},
        instance=instance)
        form2.save()
