...
def charts(request):
    ...
    highest_voted = Ticket.objects.filter(ticket_type=1).order_by('-upvotes')[:3]
    
    # Add the get the title and number of upvotes of the top 3 most upvoted feature request tickets  
    for x in highest_voted:
        chart_data.append([x.title, x.upvotes])
        
    # Get the ids of the top 3 most upvoted feature request tickets and exclude them from the query
    excluded_ids = []
    for i in highest_voted:
        excluded_ids.append(i.id)
    
    other = Ticket.objects.filter(ticket_type=1).exclude(id__in=excluded_ids).order_by('-upvotes')
    ...
    highest_voted_bugs = Ticket.objects.filter(ticket_type=2).order_by('-upvotes')[:3]
    
    # Add the get the title and number of upvotes of the top 3 most upvoted bug report tickets  
    for x in highest_voted_bugs:
        chart_data2.append([x.title, x.upvotes])
        
    # Get the ids of the top 3 most upvoted bug report tickets and exclude them from the query
    excluded_ids_bugs = []
    for i in highest_voted_bugs:
        excluded_ids_bugs.append(i.id)
    
    other_bugs = Ticket.objects.filter(ticket_type=2).exclude(id__in=excluded_ids_bugs).order_by('-upvotes')
